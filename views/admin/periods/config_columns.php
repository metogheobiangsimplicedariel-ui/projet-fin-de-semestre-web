                ...
                <?php else: ?>
                    <div class="divide-y divide-gray-100">
                        <?php foreach ($matieres as $m): ?>
                            ...
                            ?>
                            <a href="index.php?page=admin_config_cols&periode=<?= $periode['id'] ?>&matiere=<?= $m['id'] ?>"
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div class="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                            ...
                            <div class="flex gap-2">
                                <a href="index.php?page=admin_formula&periode=<?= $periode['id'] ?>&matiere=<?= $current_matiere_id ?>"
                                    class="bg-indigo-50 border border-indigo-200 text-indigo-700 px-3 py-1 rounded text-xs font-bold hover:bg-indigo-100 transition shadow-sm flex items-center">
                                    <i class="fa-solid fa-calculator mr-1"></i> Éditer la Formule
                                </a>
                                <form method="POST" action="index.php?page=admin_calculate" class="inline">
                                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                                    <input type="hidden" name="matiere_id" value="<?= $current_matiere['id'] ?>">
                                    <input type="hidden" name="periode_id" value="<?= $periode['id'] ?>">

                                    <button type="submit" class="bg-green-600 text-white px-3 py-1 rounded text-xs font-bold hover:bg-green-700 transition shadow-sm flex items-center gap-1" title="Recalculer les moyennes de tous les étudiants">
                                        <i class="fa-solid fa-rotate"></i> Calculer
                                    </button>
                                </form>
                                    ...
                                    <?php foreach ($colonnes as $col): ?>
                                        <tr data-id="<?= $col['id'] ?>" class="hover:bg-gray-50 transition group bg-white">
                                            ...
                                            <td class="p-4 text-right flex justify-end gap-2">
                                                <button type="button"
                                                    onclick="openEditModal(<?= htmlspecialchars(json_encode($col), ENT_QUOTES, 'UTF-8') ?>)"
                                                    class="text-gray-400 hover:text-blue-600 p-2 transition">
                                                    <i class="fa-solid fa-pen"></i>
                                                </button>

                                                <form method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ?');" class="mb-0">
                                                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
...
<div id="modalEditColumn" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg">
        ...
        <form method="POST" action="index.php?page=admin_config_cols&periode=<?= $periode['id'] ?>&matiere=<?= $current_matiere_id ?>" class="space-y-4">
            <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
            <input type="hidden" name="action" value="edit_column">
            <input type="hidden" name="column_id" id="edit_column_id">

            <input type="hidden" name="matiere_id" value="<?= $current_matiere_id ?>">
            <input type="hidden" name="periode_id" value="<?= $periode['id'] ?>">
...
<script>
    ...
    // 2. Fonction pour ouvrir le modal d'édition
