...
<div class="flex h-screen bg-gray-50 font-sans overflow-hidden">
    ...
    <?php
    if (file_exists('views/prof/sidebar.php')) {
        require 'views/prof/sidebar.php';
    } else {
        // Sidebar temporaire si le fichier n'existe pas
        echo '<aside class="w-64 bg-slate-900 text-white flex flex-col hidden md:flex">';
        echo '<div class="h-20 flex items-center px-8 border-b border-slate-800"><h1 class="text-xl font-bold">Espace Prof</h1></div>';
        echo '<nav class="flex-1 px-4 py-6 space-y-2">';
        echo '<a href="index.php?page=prof_dashboard" class="flex items-center gap-3 px-4 py-3 bg-blue-600 rounded-xl text-white shadow-lg shadow-blue-900/20"><i class="fa-solid fa-chalkboard-user"></i> <span class="font-bold">Mes Matières</span></a>';
        echo '<a href="index.php?page=logout" class="flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition"><i class="fa-solid fa-right-from-bracket"></i> <span class="font-medium">Déconnexion</span></a>';
        echo '</nav></aside>';
    }
    ?>
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition duration-200 flex flex-col h-full group">
                            ...
                            <div class="p-4">
                                <?php if ($is_open && !empty($m['colonnes'])): ?>
                                    <a href="index.php?page=prof_saisie&matiere_id=<?= $m['id'] ?>&periode=<?= $periode['id'] ?>"
